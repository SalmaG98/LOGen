...
import warnings
import numpy as np
from logen.datasets.dataloader_sloper4d import Sloper4DObjectsSet

warnings.filterwarnings('ignore')

__all__ = ['Sloper4DObjectsDataModule']

class Sloper4DObjectsDataModule(LightningDataModule):
    ...
    def build_dataset(self, split, excluded_tokens=None, permutation=None, n_samples=None):
        collate = Sloper4DObjectCollator()

        data_set = Sloper4DObjectsSet(
    ...
    def train_dataloader(self, shuffle=True):
        ...
        data_set = Sloper4DObjectsSet(
                data_dir=self.cfg['data']['data_dir'], 
                split='train', #SG: not used for now 
                input_channels=self.cfg['model']['in_channels']
            )
